...
class Market extends IController
{
    ...
    //邮件营销添加
    function address_upd()
    {
        $ticketObj = new IModel('youjian');
    ...
    //邮件营销展示
    public function address_showtime()
    {
        $ticketObj = new IModel('youjian');
        $arr = $ticketObj->query();
        return $arr;
...
//批量发送邮件
    public function doInvite()
    {
        ...
        $ticketObj = new IModel('youjian');
        $arr = $ticketObj->query("id=$id");
        $ticketObj = new IModel('user');
        $list = $ticketObj->query("id<8");
        //var_dump($list);die;
        foreach($list as $k=>$v){
            $mail=$v['email'];
            $smtp = new SendMail();
