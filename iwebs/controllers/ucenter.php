...
class Ucenter extends IController
{
    ...
    public function doInvite()
    {
        ...
        {
            $title    = 'email test';
            $content  = '您好，这是来自iWebShop系统的测试邮件，如果您能收到此邮件那么恭喜您，
            系统邮件服务正常。
            http://www.przl.com/iwebs/index.php?controller=simple&action=reg&uid='.$this->user['user_id'];
            if($smtp->send($mail,$title,$content))
            {
