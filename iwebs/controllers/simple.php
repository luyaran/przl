...
public  function reg()
{
    $userid   = IFilter::act(IReq::get('uid','get'));
    if($userid){
        $this->uid = $userid;
        $this->redirect('reg');
    }else{
    ...
    function reg_act()
    {
    	...
    	$userid   = IFilter::act(IReq::get('uid','post'));
    ...
    function cart3()
    {
        ...
        $count = count($order->query("user_id=$user_id and pay_status=0"));
        if($count>=3){
            $member = new IModel('member');
            $memberdel=array("status"=>3);
            $member->setData($memberdel);
            $member->update("user_id = '$user_id'");
        }
...
//				$content = smsTemplate::findPassword(array('{mobile_code}' => $mobile_code));
    ...
    public function seller()
    {
        ...
        $siteConfigObj = new Config("site_config");
        $site_config   = $siteConfigObj->getInfo();
        unset($site_config['email_type']);
        unset($site_config['email_safe']);
        unset($site_config['mail_address']);
        unset($site_config['smtp']);
        unset($site_config['smtp_user']);
        unset($site_config['smtp_pwd']);
        unset($site_config['smtp_port']);
        unset($site_config['order_by']);
        unset($site_config['order_type']);
        unset($site_config['list_type']);
        unset($site_config['auto_finish']);
        unset($site_config['index_slide']);
        unset($site_config['url']);
        unset($site_config['site_footer_code']);
        unset($site_config['tax']);
        unset($site_config['stockup_time']);
        unset($site_config['index_seo_title']);
        unset($site_config['index_seo_keywords']);
        unset($site_config['index_seo_description']);
        unset($site_config['goods_no_pre']);
        //var_dump($site_config);die;
        $arr=implode(",",$site_config);
        $list=explode(",",$arr);
        $array[]=$list;
        //var_dump($array);die;
        $this->list=$array;
...
//        );
	...
	public function seller_reg()
	{
        ...
        $longitude    = IFilter::act(IReq::get('longitude'));
        $word_wile    = IFilter::act(IReq::get('word_wile'));
		...
		$sellerRow['longitude']    = $longitude;
		$sellerRow['create_time'] = ITime::getDateTime();

		$sellerDB->setData($sellerRow);
		$res=$sellerDB->add();
        if($res){
            $testObj = new IModel("category");
            $word_wile     = IFilter::act(IReq::get('word_wile'));
            $test = array(
                'name'=>$word_wile,
                'seller_id'=>$res
