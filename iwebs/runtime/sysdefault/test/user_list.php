...
<form action="<?php echo IUrl::creatUrl("/member/seller_del");?>" method="post" name="seller_list" onsubmit="return checkboxCheck('id[]','尚未选中任何记录！')">
    <div class="content">
        <table class="list_table">
            ...
            <tr sid="<?php echo isset($item[id])?$item[id]:"";?>">
                ...
                <td title="<?php echo isset($item['mobile'])?$item['mobile']:"";?>" class="mob"><span class="mobile"><?php echo isset($item['mobile'])?$item['mobile']:"";?></span></td>
...
<script>
    $(function(){
        $(document).on("click",".mobile",function(){
            ...
            //alert(mobile);
            $(this).parent().html('<input type="text" name="mobiles" value="'+mobile+'"/>');
        });
        $(document).on("blur","input[name='mobiles']",function(){
            var mm=$(this).val();
            var sid = $(this).parents("tr").attr("sid");
            //alert(sid);
            $.getJSON("<?php echo IUrl::creatUrl("/member/ajax_seller_mobile");?>",{"id":sid,"mobile":mm});
            $(this).parent().html("<span class='mobile'>"+mm+"</span>");
        })
    })
</script>
