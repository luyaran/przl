	...
	<form method='post' action='<?php echo IUrl::creatUrl("/market/regiment_del");?>'>
		<table class="list_table">
			...
			<tbody>
				...
				<?php $regimentDB = new IQuery("regiment");$regimentDB->order = "sort asc,id desc";$regimentDB->page = "$page";$items = $regimentDB->find(); foreach($items as $key => $item){?>
				<?php $seller_id = $item['seller_id']?>
				<?php $query = new IQuery("seller");$query->where = "id = $seller_id";$items = $query->find(); foreach($items as $key => $sellerRow){?><?php }?>
				<tr>
					<td><input type='checkbox' name='id[]' value='<?php echo isset($item['id'])?$item['id']:"";?>' /></td>
					<td><a href='<?php echo IUrl::creatUrl("/site/groupon/id/".$item['id']."");?>' target='_blank' class="orange" title="<?php echo isset($item['title'])?$item['title']:"";?>"><?php echo isset($item['title'])?$item['title']:"";?> 【<?php if(isset($sellerRow)){?><?php echo isset($sellerRow['true_name'])?$sellerRow['true_name']:"";?><?php }else{?>平台自营<?php }?>】</a></td>
...
<?php echo $regimentDB->getPageBar();?>
