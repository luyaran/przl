...
<div class="container">
	...
	<?php $seller_id = IFilter::act(IReq::get('id'),'int')?>
<?php $sellerRow=Api::run('getSellerInfo',$seller_id)?>
<?php if(!$sellerRow){?>
<?php IError::show(403,'商户信息不存在')?>
<?php }?>
...
<div class="wrapper clearfix container_2">
	...
	<div class="sidebar f_l">
		<div class="box m_10">
			...
			<div class="content">
				<div class="c_box">
					<dl class="clearfix">
						<dt><strong><?php echo isset($sellerRow['true_name'])?$sellerRow['true_name']:"";?></strong></dt>
					</dl>

					<p> 评分：<span class="grade"><i style="width:<?php echo Common::gradeWidth(statistics::gradeSeller($sellerRow['id']));?>px"></i></span></p>
					...
					<?php if($sellerRow['server_num']){?>
					<p> 商户联系：<?php Sonline::qqShow($sellerRow['server_num'])?></p>
					<?php }?>
				</div>
			</div>
		</div>
		...
		<div class="box m_10">
			...
			<div class="content">
				<ul class="ranklist" id="ranklist">
					<?php foreach(Api::run('getGoodsListBySellerid',array('#seller_id#',$seller_id),10) as $key => $item){?>
		...
		<div class="box m_10">
			...
			<div class="cont">
				<ul class="ranklist">
					...
					<?php foreach(Api::run('getGoodsListBySelleridList',array('#seller_id#',$seller_id),10) as $key => $item){?>
	...
	<div class="main f_r">
		...
		<?php $goodsObj = search_goods::find(array('seller_id' => $seller_id));$resultData = $goodsObj->find();?>
		<?php if($resultData){?>
		...
		<ul class="display_list clearfix m_10">
			...
			<li class="clearfix <?php echo search_goods::getListShow(IFilter::act(IReq::get('show_type')));?>">
				<div class="pic">
					<a href="<?php echo IUrl::creatUrl("/site/products/id/".$item['id']."");?>"><img src="<?php echo IUrl::creatUrl("/pic/thumb/img/".$item['img']."/w/".$listSize['width']."/h/".$listSize['height']."");?>" width="<?php echo isset($listSize['width'])?$listSize['width']:"";?>" height="<?php echo isset($listSize['height'])?$listSize['height']:"";?>" alt="<?php echo isset($item['name'])?$item['name']:"";?>" title="<?php echo isset($item['name'])?$item['name']:"";?>" /></a>
				</div>
