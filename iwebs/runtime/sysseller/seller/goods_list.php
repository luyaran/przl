...
<head>
	...
	<script type='text/javascript' src="<?php echo IUrl::creatUrl("")."views/".$this->theme."/javascript/html5.js";?>"></script>
...
<body>
	...
	<header id="header">
		<hgroup>
			...
			<div class="btn_view_site"><a href="<?php echo IUrl::creatUrl("/test/show/id/".$seller_id."");?>" target="_blank">商家主页</a></div>
	...
	<aside id="sidebar" class="column">
		...
		<ul class="toggle">
			...
			<li class="icn_photo"><a href="<?php echo IUrl::creatUrl("/seller/bill_list");?>">货款结算申请</a></li>
		</ul>
		...
		<ul class="toggle">
			...
			<li class="icn_photo"><a href="<?php echo IUrl::creatUrl("/seller/share_list");?>">平台共享商品</a></li>
		...
		<ul class="toggle">
			...
			<li class="icn_categories"><a href="<?php echo IUrl::creatUrl("/seller/pro_rule_list");?>">促销活动列表</a></li>
		</ul>

		<h3>配置模块</h3>
		<ul class="toggle">
			<li class="icn_categories"><a href="<?php echo IUrl::creatUrl("/seller/delivery");?>">物流配送</a></li>
			<li class="icn_categories"><a href="<?php echo IUrl::creatUrl("/seller/ship_info_list");?>">发货地址</a></li>
	...
	<section id="main" class="column">
		<?php $seller_id = $this->seller['seller_id'];$searchArray = Util::getUrlParam('search')?>
<?php $searchParam = http_build_query($searchArray)?>
<?php $condition = Util::search(IReq::get('search'));$where = $condition ? " and ".$condition : "";$join = isset($searchArray['search']['ce.category_id=']) ? " left join category_extend as ce on ce.goods_id = go.id " : ""?>
	...
	<form action="<?php echo IUrl::creatUrl("/seller/goods_del");?>" method="post" name="goodsForm">
		<table class="tablesorter" cellspacing="0">
			...
			<tbody>
				...
				<?php $goodsObject = new IQuery("goods as go");$goodsObject->where = "go.seller_id = $seller_id $where";$goodsObject->page = "$page";$goodsObject->join = "$join";$goodsObject->fields = "distinct go.id, go.*";$items = $goodsObject->find(); foreach($items as $key => $item){?>
				<tr>
	...
	<div class="module_content">
		...
		<fieldset>
			...
			<div class="box">
				...
				<input type="button" class="alt_btn" onclick="selectGoodsCategory('<?php echo IUrl::creatUrl("/block/goods_category/type/radio");?>','search[ce.category_id=]')" value="设置分类" />
			</div>
		</fieldset>
...
{
	art.dialog.open('<?php echo IUrl::creatUrl("/goods/collect_import/seller_id/".$seller_id."");?>',{
		id:'collect_import',
	    title:'商品采集器',
	    okVal:'开始采集',
	    ok:function(iframeWin, topWin){
...
{
	art.dialog.open('<?php echo IUrl::creatUrl("/goods/csvImport/seller_id/".$seller_id."");?>',{
		id:'csvImport',
	    title:'导入csv商品数据包',
	    okVal:'开始导入',
	    ok:function(iframeWin, topWin){
