		...
		else if($orderRow['pay_status'] == 0)
		{
			...
			if($orderRow['user_id'])
			{
				...
				if($memberRow)
				{
                    ...
                    $exp=$memberRow['exp'] + $orderRow['exp'];
                    $testObj = new IModel("user_group");
                    $arr = $testObj->query();
                    foreach($arr as $k => $v){
                        if($exp >= $v['minexp'] && $exp < $v['maxexp']){
                            $memberData['group_id']= $v['id'];
                        }
                    }
//                    $log = new log();
//                    $log->write('sms',array(var_dump($arr)));
	...
	public static function refund($refundId,$authorId,$type = 'admin',$isNotBalance = 0)
	{
		...
		$amount         = $orderGoodsRow['goods_nums'] * $orderGoodsRow['real_price'];
